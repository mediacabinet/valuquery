...
class AttributeSelectorParser extends AbstractParser
{
    ...
    protected function parseOperator()
    {
        ...
        if ($operator === Attribute::OPERATOR_NEGATION && $this->current() !== Attribute::OPERATOR_EQUALS) {
            $operator .= $this->current();
            $this->next();
        }
        ...
        if ($this->current() == Attribute::OPERATOR_EQUALS) {
            $operator .= $this->current();
            $this->next();
        } elseif (!in_array($operator, $supportedOperators)) {
    ...
    protected function parseValue()
    {
        ...
        if (!$quoted) {
            ...
            if ($canonicalValue === 'true') {
                $value = true;
            } elseif ($canonicalValue === 'false') {
                $value = false;
            } elseif ($canonicalValue === 'null') {
                $value = null;
