...
class QueryHelperTest extends AbstractTestCase
{
    ...
    public function testQueryDBRefAssociation()
    {
        ...
        $this->assertEquals([$dbRef], $result);
    }

    public function testQuerySimpleAssociation()
    {
        $sound = $this->createTestEntity('Sound',['says' => 'Roaar!']);
        $superCat = $this->createTestEntity('Cat',['sound' => $sound, 'name' => 'Supercat']);

        $result = $this->queryHelper->query('*', 'sound');
        $this->assertEquals([$sound->id], $result);
    }
